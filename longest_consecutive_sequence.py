...
class Solution:
    def extend_sequences(self, set_one: ConsecutiveSet, set_two: ConsecutiveSet):
        set_one.set_upper = (
            set_two.set_upper
            if set_two.set_upper > set_one.set_upper
            else set_one.set_upper
        )
        set_one.set_lower = (
            set_two.set_lower
            if set_two.set_lower < set_one.set_lower
            else set_one.set_lower
        )
        set_two.set_upper = (
            set_one.set_upper
            if set_one.set_upper > set_two.set_upper
            else set_two.set_upper
        )
        set_two.set_lower = (
            set_one.set_lower
            if set_one.set_lower < set_two.set_lower
            else set_two.set_lower
        )

    def check_if_consecutive(
        self, num: int, seq_map: Dict[int, ConsecutiveSet]
    ) -> None:
    ...
    def longestConsecutive(self, nums: List[int]) -> int:
        seq_map: Dict[int, ConsecutiveSet] = {
            num: ConsecutiveSet(set_lower=num, set_upper=num) for num in nums
        }
        if len(nums) < 2:
            return len(nums)
        for _ in range(1):
